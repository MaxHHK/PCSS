...
%         end
        ...
        function DrawScheme(obj,PopDec)
            N      = size(PopDec,1);
...
%                 plot(plot_x, consume_sum);
                plot(plot_x, consume_sum, 'LineWidth', 2);
            end
            ann = ['总用气量=',num2str(PopObj(1)), '波动', num2str(PopObj(2))];
%             annotation('textbox',[0.2,0.2,0.1,0.1],'LineStyle','-','LineWidth',2,'String',ann);
%             legend(ann);
            xlabel('生产周期', 'fontsize', 20, 'FontName', '宋体');
            ylabel('管道用气量', 'fontsize', 20);
